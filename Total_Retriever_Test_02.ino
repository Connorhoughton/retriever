            ...
            case 'D':  //enter
              ...
              while (boolRunVFD == true) {
                ...
                if (directionOneAbs > directionTwoAbs) { //If direction two is shorter, travel in reverse
                  ...
                  //Run VFD Backward until newLocation = locationCalculation (While loop?)
...
void update_Location() {
  totalPulses = abs(pulses);
  if (i == 0) {
    hookCalculation = float(TotalLength) / float((lastLocation + 1) - (firstLocation));
    i++;
  }
  if (totalPulses >= TotalLength) {
    pulses = 1;
  ...
  locationCalculation = round(lastLocation - (totalPulses / hookCalculation));
